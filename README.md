# ✈️ Aviation Chatbot - RAG-Based Technical Documentation Assistant
...
- **🔍 Semantic Search**: Vector-based similarity search using pgvector for accurate document retrieval
- **🤖 AI-Powered Answers**: Context-aware responses generated by Google Gemini 2.5 Flash
- **📤 Dynamic PDF Upload**: Users can upload new documents without restarting the system
- **📚 Source Citations**: Every answer includes references to source documents and page numbers
- **🔒 Privacy Protection**: Prevents leakage of internal document details for off-topic queries
- **⚡ Real-time Processing**: Live progress tracking for PDF ingestion
- **💾 Persistent Storage**: PostgreSQL database for reliable, scalable data management

### User Experience

- **💬 Chat Interface**: Clean, modern Streamlit UI similar to ChatGPT
- **📊 System Monitoring**: Real-time database statistics and document management
- **🎨 Dark Theme**: Eye-friendly interface with high contrast
- **📱 Responsive Design**: Works on desktop and tablet devices
- **🗑️ Document Management**: Delete and re-upload documents as needed
...
│  • Chat Input/Output  • PDF Upload  • Settings  • Stats     │
└──────────────────────┬──────────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────────┐
...
│  • Document Chunks     │  │  • Answer Generation │
│  • Vector Embeddings   │  │  • Context Synthesis │
│  • Metadata            │  │                      │
└────────────────────────┘  └──────────────────────┘
               ▲
               │
...
│  (pdfplumber → RecursiveTextSplitter → Sentence            │
...
1. **Click**: "📤 Upload New PDF" in the sidebar
2. **Select**: Choose a PDF file (max 200MB)
3. **Process**: Click "🚀 Process PDF"
4. **Monitor**: Watch real-time progress bar
5. **Query**: Ask questions about the new document immediately

### Managing Documents

1. **View Stats**: Check "📊 System Status" for database info
2. **Delete Documents**: Use "🗑️ Document Management" section
...
[Vector Similarity Search] (PostgreSQL + pgvector)
    ├─ Find top K most similar chunks (K=8 default)
...
[Generate Answer] (Gemini 2.5 Flash)
    ...
    ├─ User query
    └─ Privacy protection for off-topic queries
    ↓
[Format Response]
    ├─ Answer text
    ├─ Source citations
